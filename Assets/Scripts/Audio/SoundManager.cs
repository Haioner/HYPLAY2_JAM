...
public class SoundManager : MonoBehaviour
{
    ...
    public static void PlayAudioClipRandomPitch(AudioClip clip, Vector2 minMax)
    {
        _audioSource.volume = 1f;
        float randPitch = Random.Range(minMax.x, minMax.y);
    ...
    public static void PlayContinousAudioClipVolumeAndRandomPitch(AudioClip clip, float volume, Vector2 pitch)
    {
        _continousAudioSource.volume = volume;
        float randPitch = Random.Range(pitch.x, pitch.y);
        _continousAudioSource.pitch = randPitch;
        _continousAudioSource.PlayOneShot(clip);
    }
